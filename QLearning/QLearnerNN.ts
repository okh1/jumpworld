...
module QLearning {
    ...
    export class QLearnerNN implements IQLearner {
        ...
        training_error: number; //Cost, error in predicting qvalues by neural network
        average_reward: number; //Average reward
        ...
        constructor(world: QLearning.World, numberofstatefeatures: number, layer_defs?: any, opt_trainer?: any) {
            ...
            if (opt_trainer == null) //Default trainer options
                opt_trainer = { learning_rate: 0.001, method: 'sgd', batch_size: 64, l2_decay: 0.01 };
            ...
            if (this.memory.length > this.start_learn_threshold) {
                ...
                for (var i = 0; i < this.trainer.batch_size; i++) {
                    ...
                    if (isNaN(maxact.value))
                        console.log("nan");
                    var value = xp.reward0 + this.gamma * this.decode(maxact.value);
                    if (!this.world.isfinalreward(xp.reward0))
                    ...
                    cost += loss.loss; rew += xp.reward0;
                }
                this.training_error = cost / this.trainer.batch_size;
                this.average_reward = rew / this.trainer.batch_size;
            }
        }
        ...
        private encode(x: number) {
            return x;
            /*var ymax = 1; var ymin = -1; var xmin = -100; var xmax = 1;
            return (ymax - ymin) * (x - xmin) / (xmax - xmin) + ymin;*/
        }

        private decode(x: number) {
            return x;
            /*var ymax = 1; var ymin = -100; var xmin = -1; var xmax = 1;
            return (ymax - ymin) * (x - xmin) / (xmax - xmin) + ymin;*/
        }
